...
func _show_announcement(wave_number: int):
	...
	if wave_number == 20:
		label.text = "BOSS - Wave 20"
	label.position = Vector2(440, 300)
	label.add_theme_font_size_override("font_size", 64)
	label.add_theme_color_override("font_color", Color.GOLD if wave_number < 20 else Color.RED)
	add_child(label)
	...
	tween.tween_callback(func(): hide(); modulate.a = 1.0)
